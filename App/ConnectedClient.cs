...
namespace App
{
    ...
    public class ConnectedClient
    {
        ...
        private readonly BufferBlock<ControlMessage> _controlMessageQueue = new BufferBlock<ControlMessage>();
        ...
        {
            _controlMessageQueue.Post(new ControlMessage.RoomMessage(message, sender));
        }

        // Instructs the client to exit
        public void Exit()
        {
            _controlMessageQueue.Post(new ControlMessage.Stop());
        }

        async public Task MainLoop(CancellationToken cancellationToken)
        {
            try
            {
                cancellationToken.Register(() => ServerExit());
                Task? task1 = null;
                Task? task2 = null;
                while (!_exiting)
                {
                    ...
                    {
                        task1 = RemoteReadLoop();
                    }

                    if(task2 == null || task2.IsCompleted)
                    {
                        task2 = _controlMessageQueue.ReceiveAsync().ContinueWith(controlMessageTask =>
                        {
                            var controlMessage = controlMessageTask.Result;
                            try
                            {
        ...
        async private Task RemoteReadLoop()
        {
            try
            {
                while (!_exiting)
                {
                    ...
                    _controlMessageQueue.Post(new ControlMessage.RemoteLine(line));
                }
            }
            catch (Exception e)
            {
            ...
            finally
            {
                if (!_exiting)
                {
                    _controlMessageQueue.Post(new ControlMessage.RemoteInputEnded());
                }
            }
